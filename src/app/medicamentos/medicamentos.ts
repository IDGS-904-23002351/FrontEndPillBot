...
import {
  ...
  TipoPresentacion,
  UnidadMedida
} from '../models/medicamentos.models';
...
export class MedicamentosComponent implements OnInit {
  ...
  private readonly apiPresentaciones = `${environment.apiUrl}/api/presentaciones`;
  private readonly apiUnidadesMedida = `${environment.apiUrl}/api/unidadesMedida`;
  ...
  medicamentosFiltrados = computed(() => {
    ...
    // Si es médico, solo ve los que él registró
  ...
  guardarMedicamento(): void {
    ...
    if (this.modal() === 'crear') {
      ...
      this.http.post<{ mensaje: string; idMedicamento: number }>(this.apiMedicamentos, nuevoMedicamento).subscribe({
        next: (respuesta) => {
          // Guardamos el ID como "propio" del médico logueado
