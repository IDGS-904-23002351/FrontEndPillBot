...
import { Receta, Cliente } from '../models/receta.models';
...
export class RecetaComponent implements OnInit {
  ...
  private cargarRecetasCliente(): void {
    ...
    const url = `${this.apiRecetas}/cliente/${idUsuario}`;
    this.http.get<Receta[]>(url).subscribe({
      ...
      error: (error) => {
        console.error('Error al consultar las recetas del cliente:', error);

        this.recetas.set([]);
        this.cargando.set(false);
        this.errorCarga.set(
          error?.error?.mensaje ?? 'No se pudieron cargar tus recetas.'
        );
      }
    });
  }
